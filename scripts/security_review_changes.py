...
Env:
  CS_SKIP_SECURITY_REVIEW=1  bypass entirely (escape hatch)
  CS_SKIP_CLAUDE_REVIEW=1    skip the semantic Claude pass, static rules only
"""
...
def run_claude_review(diff: str) -> tuple[str | None, str | None]:
    ...
    if os.environ.get("CS_SKIP_CLAUDE_REVIEW") == "1":
        return None, "CS_SKIP_CLAUDE_REVIEW=1"
    claude = find_claude_cli()
    if not claude:
        return None, "claude CLI not found (set CS_CLAUDE_BIN or install on PATH)"
    if not diff.strip():
...
def main() -> int:
    ...
    if os.environ.get("CS_SKIP_SECURITY_REVIEW") == "1":
        print("[security-review] skipped (CS_SKIP_SECURITY_REVIEW=1)", file=sys.stderr)
        return 0
