...
class TestDeriveEpicStatus(unittest.TestCase):
    ...
    def test_open_and_closed_is_active(self):
        # Mixed open + closed = work has progressed even without an
        # explicitly active task; derive_epic_status's contract
        # ([housekeep.py docstring]) treats any mixing as "active" /
        # in-progress.
...
class TestGenerateEpicsMd(unittest.TestCase):
    ...
    def test_epic_with_prereqs_produces_graph_edges(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            content = self._run(t)
            # housekeep emits LR-oriented mermaid for task chains so the
            # arrows read in the same direction as the prereqs list.
...
class TestInitEndToEnd(unittest.TestCase):
    ...
    def test_init_then_apply_with_a_task(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Mermaid kanban node IDs use underscore form; see KanbanMd tests.
