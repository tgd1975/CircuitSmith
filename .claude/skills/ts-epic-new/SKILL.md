...
title: <Short title>    # human-readable heading — rendered as H1 by markdownlint
...
Do **not** add a body `# <Short title>` heading. The frontmatter
`title:` is already an H1 (per markdownlint's MD025 rule), so a body
H1 would be a duplicate top-level heading. Body content starts directly
with the motivation paragraph; the first body heading is `## Tasks`.
