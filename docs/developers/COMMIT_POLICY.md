...
CircuitSmith enforces a stricter commit protocol than most Python
projects:
...
1. `scripts/commit-pathspec.sh` runs, writes
   `.git/cs-commit-token` with the form `<pid> <nonce> <unix-ts>`.
...
5. If valid → hook continues with its lint / portability / commit-policy
...
## Bypass policy — `CS_COMMIT_BYPASS`

There is one escape hatch for the provenance-token check:

```bash
CS_COMMIT_BYPASS="<reason>" git commit ...
```

Set the env var to a non-empty reason; the hook accepts the commit
and **logs the bypass** to `.git/cs-commit-bypass.log` with timestamp,
...
boundaries. This mirrors the upstream policy in
AwesomeStudioPedal's `RELEASE_CHECKLIST.md` (which scopes it to
release boundaries) — CircuitSmith tightens it to every merge.
...
Generated with …" — none of these appear in CircuitSmith's commit
