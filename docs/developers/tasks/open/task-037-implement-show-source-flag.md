...
- [ ] Block syntax accepts `show_source` as a flag (e.g. ` ```circuit show_source ` or YAML-frontmatter style).
- [ ] When set, output includes a `<details>` block with the verbatim source YAML.
- [ ] When absent, output is unchanged from TASK-036 baseline.
- [ ] Flag is documented in `docs/circuit-yaml.md` with a worked example.
