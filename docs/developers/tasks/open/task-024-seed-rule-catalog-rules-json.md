...
- [ ] 15 entries seeded, one per shipped check code (S1–S5 + E1–E10).
- [ ] Every entry has all six required fields populated.
- [ ] Every `source_of_truth` URL is reachable at seed time (validated manually before commit).
- [ ] Every `heuristic` carries a precision disclaimer (validated by TASK-025).
- [ ] No copied text from sources — every entry is a reformulation.
