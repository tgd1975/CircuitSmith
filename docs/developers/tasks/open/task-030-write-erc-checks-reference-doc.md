...
- [ ] All 15 check codes have their own section in `erc-checks.md`.
- [ ] Each section has all five subsections populated.
- [ ] Each suppression example is a valid `.circuit.yml` snippet that the schema accepts.
- [ ] Cross-references to `rules.json` entries are valid (every `enforced_by` resolves).
