...
- [ ] `validate_catalog.py` runs cleanly on the seeded `rules.json` from TASK-024.
- [ ] Each of the five checks has a targeted failure fixture; deliberately breaking each one produces a clear error.
- [ ] The `category`-invariant lint allows reads inside `layout_engine/` (the rule's intended scope) and flags reads anywhere else.
- [ ] URL reachability check supports an env-var allowlist for offline CI runs (`CS_CATALOG_OFFLINE=1` skips URL checks).
- [ ] CI integration: a new GitHub Actions step runs this validator and fails the build on errors.
