...
- [x] `renderer.py` calls `erc_engine.run(net_graph)` after schema validation, before kernel invocation.
- [x] Any ERROR-level finding aborts the renderer with exit code 1 (or a documented code distinct from schema/rubric exits).
- [x] WARNING-level findings are recorded in the report but do not abort.
- [x] The pipeline order matches `idea-001-circuit-skill.md §Pipeline`.
