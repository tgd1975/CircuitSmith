...
- [x] `BACKLOG.md` lists at least 15 candidate educational rules across the five categories.
- [x] Each entry has a one-line description and an indicative priority (high/medium/low).
- [x] The file documents the authoring workflow (one rule per PR, reviewer checks source-of-truth link).
- [x] No deep specifications — this is a queue, not a design doc.
