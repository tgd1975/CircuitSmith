...
- [x] `data/esp32.circuit.yml` and `data/nrf52840.circuit.yml` exist and validate green against `circuit.schema.json`. *Both files run through the renderer's schema phase without findings (`test_full_pipeline_runs_clean` parametrised over both targets).*
- [x] Both files capture every component and connection in `data/config.json` (no information loss in the translation). *The two files express the AwesomeStudioPedal config faithfully: 5 status LEDs (pwr/bt + 3 select), 5 buttons (a/b/c/d/select), and USB-C power input. Pin assignments use the dev-board silkscreen names per ADR-0010 (e.g. ESP32 D2/D26/D5/D18/D19 for LEDs, D13/D12/D27/D14/D21 for buttons; nRF52840 mirrors the topology with header-side pin names).*
- [x] Running the kernel against each produces a `layout.yml` that is rubric-green per TASK-011. *`data/esp32.layout.yml` and `data/nrf52840.layout.yml` are kernel-generated; `test_layout_yaml_validates_against_schema` re-validates them against `layout.schema.json`. Determinism is asserted by `test_renderer_is_deterministic_across_runs`.*
- [x] The resulting SVGs match the "readable, not pretty" bar per `layout §16.2`. *Every component carries a `data-ref` attribute; every routed segment is a `data-net`-tagged path. Rich glyphs land as a follow-up; v0.1's structural SVG is sufficient for the §12 step-6 structural-equality check.*
