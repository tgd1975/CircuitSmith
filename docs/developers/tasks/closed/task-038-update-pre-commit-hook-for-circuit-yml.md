...
- [x] Editing `data/esp32.circuit.yml` and staging it auto-regenerates and stages all four output artefacts.
- [x] Editing a component profile (`components/passives.py`, etc.) also triggers regeneration for any circuit using that profile.
- [x] The hook is fast enough not to be skipped — under 5 seconds on the shipped circuits (regenerate skipped if `meta.yml.fingerprint` matches the input hash).
- [x] The hook is bypassable via the documented project mechanism (`CS_COMMIT_BYPASS`).
