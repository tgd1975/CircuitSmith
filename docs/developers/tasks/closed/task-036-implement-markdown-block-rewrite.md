...
- [x] A Markdown file containing a ` ```circuit ` block renders to a page with the embedded SVG.
- [x] SVG filename includes an 8-char hex hash of the source YAML.
- [x] Stale SVGs (hash mismatch) are detected and regenerated (or the build fails on staleness in CI).
- [x] Implementation matches whichever mechanism is current per the IDEA-022 ordering.
