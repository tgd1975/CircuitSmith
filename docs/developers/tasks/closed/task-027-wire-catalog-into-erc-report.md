...
- [x] Every non-OK finding in `erc-report.md` carries the three-paragraph enrichment block.
- [x] Lookup by check code; no fallback to keyword/fuzzy matching.
- [x] A finding without a matching catalog row produces a fail-loud diagnostic naming the missing check code.
- [x] The enrichment block formatting matches the spec in `idea-001.erc-engine.md §Enriched report format` (verified by snapshot test).
