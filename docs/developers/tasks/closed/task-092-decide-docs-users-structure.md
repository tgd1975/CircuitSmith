...
- [x] Directory structure exists with the chosen layout (empty stubs
      are fine).
- [x] `README.md` has an updated documentation pointer.
- [x] Decision rationale captured — either in the commit message or
      a short ADR under `docs/developers/adr/`.

## Outcome

Option 1 chosen: `docs/users/` sibling alongside `docs/developers/`
and `docs/builders/`. Rationale captured in
[ADR-0014](../../adr/0014-docs-users-sibling-folder.md). Directory
skeleton created with stub READMEs in `docs/users/`,
`docs/users/tutorial/`, and `docs/users/examples/` — content lands
in TASK-093 onwards. Repo `README.md` gains a new "Documentation"
section that surfaces all three audiences (users / builders /
developers) above the existing "Deep-dive docs" companion table.
