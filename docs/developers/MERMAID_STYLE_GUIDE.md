...
Conventions for mermaid diagrams in CircuitSmith's developer docs.
...
| Role | Hex | Use for |
|---|---|---|
| Data node | `#dde7ee` (light slate) | Static inputs/outputs (YAML, JSON, files). |
...
linkStyle 11 stroke:#c00,stroke-width:2px,stroke-dasharray:5 3
```

(`11` is the zero-indexed position of the edge in the diagram.)

## Node-label conventions

- **Data nodes**: nouns. `".circuit.yml"`, `"NetGraph"`, `"rules.json"`.
- **Process nodes**: short verb phrases. `"schema validation"`,
  `"build NetGraph"`, `"render SVG"`.
- **Top-level subsystems**: ALL-CAPS or first-letter-cap, brief.
  `RENDERER`, `ERC`, `LAYOUT_KERNEL`. Subscripts in HTML `<sub>` tags
  for the one-line annotation.
...
````markdown
The pipeline runs left-to-right: YAML → schema validation →
NetGraph build → ERC → layout → render → meta.yml. ERC sits
strictly between NetGraph build and layout, so a malformed
circuit cannot reach the router.

```mermaid
flowchart LR
    YAML --> SCHEMA --> NETGRAPH --> ERC --> LAYOUT --> RENDER
```
````
