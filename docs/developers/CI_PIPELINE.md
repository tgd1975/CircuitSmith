...
| Doc cross-reference gate (TASK-105) | `python scripts/check_doc_references.py`. Fails when a living narrative doc has a broken relative link. Frozen records (`tasks/`, `ideas/`, `adr/`), vendored skills, code fences, and lifecycle-folder drift are out of scope; the ID / code-path / external-URL classes are opt-in flags (`--check-ids`, `--check-paths`, `--check-external`), off in CI. | A relative link target was moved or deleted. Reproduce locally with the same command; repoint the link. |
