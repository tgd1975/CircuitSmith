...
CircuitSmith ships an autonomous-loop workflow: Claude can edit files,
...
`CS_SKIP_CLAUDE_REVIEW=1` to disable the semantic pass and run static
rules only.
...
- [ ] **Personal data leak.** The maintainer's email / real name /
  phone in committed text. See
  [TASK-074](tasks/open/task-074-personal-data-leak-check-in-security-review-hook.md)
  for the planned mechanical enforcement.
...
```
edit time     →  code-owner skills (`co-netgraph`, `co-schema`, …)
...
```bash
CS_SKIP_SECURITY_REVIEW=1 git pull   # bypass the merge / pull / rebase hooks
CS_SKIP_CLAUDE_REVIEW=1 …            # static rules only, skip semantic pass
```
...
2. **File the finding as a security-tagged task.** Use `/ts-task-new`
   under EPIC-008 (architecture fitness functions) with the
   `human-in-loop: Main` field — security findings always escalate to
   the user. Describe the finding, the file:line, the affected blast
   radius, and the suggested fix.
