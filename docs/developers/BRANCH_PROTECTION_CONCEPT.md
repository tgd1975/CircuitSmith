...
CircuitSmith's branch policy (no direct commits to `main`,
...
([TASK-073](tasks/open/task-073-apply-branch-protection.md)) applies
...
- **Contributor #2 lands.** Flip "require PR review" to "yes —
  ...
  still hot-fix.
- **The skill is extracted in Phase 7 (EPIC-006).** The standalone
  repo inherits this ruleset; revisit whether the solo-posture
  concessions still hold for the extracted repo.
...
```bash
gh api -X PUT /repos/tgd1975/CircuitSmith/branches/main/protection \
   --input <json-body>
```
...
[TASK-073](tasks/open/task-073-apply-branch-protection.md)'s implementation;
...
|---|---|---|
| Epic | `release/epic-NNN-<slug>` | `release/epic-009-developer-docs-governance` |
| Chore | `chore/<scope>` | `chore/security-deny-list` |
| Fix | `fix/<scope>` | `fix/renderer-pin-clamp` |
| Refactor | `refactor/<scope>` | `refactor/erc-rule-loader` |
...
If the ruleset changes (TASK-073 amends the live config, or a
