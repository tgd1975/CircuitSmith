...
| Code-owner skills | `.claude/skills/co-<name>/SKILL.md` | One per high-blast-radius module. Authored in TASK-056. |

## How to add a code-owner skill

1. **Identify the file.** Pick a single load-bearing file (or a
   tightly coupled set — `schema/*.json`). Anything an accidental
   edit could quietly break downstream is a candidate. The initial
   set in TASK-056: `netgraph.py`, `schema/*.json`, `erc_engine.py`.
2. **Author the skill.** Create
   ...
   See [`docs/developers/ideas/archived/idea-001.<file>.md §<section>`](…).
...
guardrails live in the boundary-import contract test (TASK-050) and
the lints — not here.

## Related

- [`ADR-0007`](adr/0007-skill-directory-is-the-library.md) — why the
  skill directory is the library, which is why these reminders even
  exist (high-blast-radius files are by definition the *library*
  shape).
- TASK-055 — this mechanism.
- TASK-056 — the first three code-owner skills (`co-netgraph`,
  `co-schema`, `co-erc-engine`).
